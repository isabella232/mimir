...
const (
	...
	// Maximum bytes count in frame for streamed read response
	maxBytesInFrame = 1024 * 1024
)

// RemoteReadHandler handles Prometheus remote read requests.
func RemoteReadHandler(q storage.SampleAndChunkQueryable, lg log.Logger) http.Handler {
...
func remoteReadStreamedXORChunks(ctx context.Context, q storage.ChunkQueryable, logger log.Logger, w http.ResponseWriter, req *client.ReadRequest) {
	f, ok := w.(http.Flusher)
	if !ok {
...
func streamChunkedReadResponses(stream io.Writer, queryIndex int64, ss storage.ChunkSeriesSet) error {
	var (
